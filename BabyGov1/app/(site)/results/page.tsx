      ...
      content: (
        <div className="space-y-3">
          ...
          <ul className="list-disc pl-6 text-sm text-gray-700">
            {bcs.rationale.map((r, i) => <li key={i}>{r}</li>)}
          </ul>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {(hotels as any[]).filter(h => h.destination_id === destId).map(h => (
            <div key={h.hotel_id} className="rounded-lg border bg-white p-3">
              ...
              <ul className="mt-2 text-sm">
                {Object.entries(h.amenities).map(([k, v]) => v ? <li key={k}>✓ {k.replaceAll("_"," ")}</li> : null)}
              </ul>
            </div>
          ))}
        </div>
          ...
          {(packing as any[]).filter(p => p.season === "Mild").map((t) => (
            <div key={t.template_id} className="rounded-lg border bg-white p-3">
              ...
              <ul className="mt-2 text-sm list-disc pl-6">
                {t.items.map((it: any, idx: number) => <li key={idx}>{it.name}{it.mandatory ? " • must-have" : ""}</li>)}
              </ul>
            </div>
          ))}
        </div>
