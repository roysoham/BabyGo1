...
export default function SearchForm(){
  ...
  function onSubmit(e: React.FormEvent){
    ...
    // ---------- Fallback destination lat/lng if user didn’t choose a Places suggestion ----------
    let toLat: number|undefined;
    let toLng: number|undefined;
    if (!toId) {
      const fb = findCityByName(to);
      if (fb) { toLat = fb.lat; toLng = fb.lng; }
    }
    if (!toId && (toLat == null || toLng == null)) {
      setErr("Select a destination from suggestions or enter a known city (e.g., Paris, France).");
      return;
    }
    // -------------------------------------------------------------------------------------------
    ...
    if (!toId && toLat != null && toLng != null) {
      q.set("lat", String(toLat));
      q.set("lng", String(toLng));
    }
  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto max-w-6xl rounded-2xl border bg-white p-4 shadow">
      ...
      <div className="mt-3 flex items-center justify-end gap-3">
        <button type="submit" className="rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 px-5 py-2 font-semibold text-white shadow">
          Search
        </button>
      </div>
